...
.DESCRIPTION
    ...
    Version: 0.6.0
    Build Date: 2025-12-15 16:52:49
...
function ConvertTo-AmigaGuideCommand {
    ...
    $parsedArgs = Parse-CommandArguments -Arguments $Command.Arguments -Spec $spec -CommandName $cmdName
    
    if ($parsedArgs -eq $null) {
...
function Parse-CommandArguments {
    ...
    if ($Spec.argType -eq "font") {
        return Parse-FontArguments -Arguments $Arguments
    }
...
function Test-AGCommandSyntax {
    ...
    foreach ($cmd in $Commands) {
        ...
        $parsed = Parse-CommandArguments -Arguments $cmd.Arguments -Spec $spec -CommandName $cmdName
...
function New-AmigaGuideDocument {
    ...
    for ($i = 0; $i -lt $MarkdownDocument.Nodes.Count; $i++) {
        ...
        # Default @help and @index (if they exist as nodes)
        ...
        # T046: Default @smartwrap for automatic word wrapping
        ...
        if (-not [string]::IsNullOrWhiteSpace($node.Content)) {
            ...
            $footnotes = Parse-Footnotes -Text $node.Content
            
            # Remove footnote definitions from content (will be added at end)
            $contentWithoutDefs = Remove-FootnoteDefinitions -Text $node.Content
...
if ($Version) {
    Write-Host "MD2AG - Markdown to AmigaGuide Converter" -ForegroundColor Cyan
    Write-Host "Version: 0.6.0" -ForegroundColor White
    Write-Host "Author: MD2AG Project" -ForegroundColor Gray
    Write-Host "License: MIT" -ForegroundColor Gray
    Write-Host ""
    Write-Host "Usage: md2ag.ps1 -Input <file.md> [-Output <file.guide>]" -ForegroundColor Yellow
    Write-Host "       md2ag.ps1 <file.md>  (Output defaults to <file.guide>)" -ForegroundColor Yellow
    Write-Host "       md2ag.ps1 -Version" -ForegroundColor Yellow
...
try {
    ...
    if (-not $validation.IsValid) {
        if ($Strict) {
            ...
            foreach ($error in $validation.Errors) {
                Write-Host "  WARNING: Line $($error.Line) - $($error.Message)" -ForegroundColor Yellow
                if ($error.Context) {
                    Write-Host "    Context: $($error.Context)" -ForegroundColor Gray
                }
                if ($error.Suggestion) {
                    Write-Host "    Auto-fix: $($error.Suggestion)" -ForegroundColor Cyan
                }
            }
    ...
    # Display warnings
    if ($validation.Warnings.Count -gt 0) {
        Write-Verbose "Found $($validation.Warnings.Count) warning(s)"
        foreach ($warning in $validation.Warnings) {
            Write-ValidationWarning -Warning $warning
        }
    }
    ...
    try {
        ...
        # Validate broken links (now just warnings)
    ...
    if ($Mode -eq 'Merge' -and $allDocuments.Count -gt 1) {
        ...
        $allNodes = $mergeResult.Nodes
        $globalCommands = $mergeResult.GlobalCommands
        
        Write-Verbose "├─ Merged: $($allNodes.Count) nodes from $($allDocuments.Count) documents"
    }
    ...
    elseif ($Mode -eq 'Optimized' -and $allDocuments.Count -gt 1) {
        ...
        $filteredNodes = Invoke-OptimizedMode -Context $context -AllNodes $allNodes
        $allNodes = $filteredNodes
        
        Write-Verbose "├─ Optimized: $($allNodes.Count) reachable nodes"
    }
    else {
    ...
    Write-Verbose "├─ Parsed: $($content.Length) characters, $($allNodes.Count) nodes"
    ...
    if (-not $outputValidation.IsValid) {
        ...
        foreach ($error in $outputValidation.Errors) {
            Write-Host "  ERROR: $($error.Message)" -ForegroundColor Red
            if ($error.Context) {
                Write-Host "    Context: $($error.Context)" -ForegroundColor Gray
            }
            if ($error.Suggestion) {
                Write-Host "    Suggestion: $($error.Suggestion)" -ForegroundColor Cyan
            }
        }
        exit $ExitCodes.InvalidSyntax
    }
    ...
    if ($outputValidation.Warnings.Count -gt 0) {
        Write-Host ""
        Write-Host "Output warnings: $($outputValidation.Warnings.Count)" -ForegroundColor Yellow
